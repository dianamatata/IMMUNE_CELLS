...
for cell_type in ${Cell_Types[@]}; do
    for histone in ${Histone_Modifications[@]}; do
        ...
        # command in cluster
        sbatch -J $FILENAME.job --partition=mono-EL7 --time=2:00:00  --mem-per-cpu=70000000 -o $FILENAME.out -e $FILENAME.err --wrap="$cmd"

    done
done
# with --time=1:00:00 only 2 bam files substampled are generated
