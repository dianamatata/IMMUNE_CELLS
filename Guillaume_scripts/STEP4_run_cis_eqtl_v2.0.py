...
vcf_file = '/data/unige/funpopgen/2backup/grey2/external_data/Blueprint_immune_cells/EGAD00001002663/All_chr.BPWP10_13_12_15.vcf.gz'
...
for i in covariates:
	if i>=0:
		genopheno = pd.concat([genopca.ix[0:2,:], phenopca.ix[0:i,:]], join="inner")
		if len(sys.argv)>3:
			genopheno = pd.concat([genopheno,covariate_data],join="inner")
	else:
		if len(sys.argv)>3:
			genopheno = pd.concat([genopca.ix[0:2,:],covariate_data],join="inner")
		else:
			genopheno = genopca.ix[0:2,:]
	
	genopheno.to_csv(outfolder+'/covariates_'+str(i+1)+'.txt',sep='\t',float_format='%.3f',index=False)
	for j in range(1,nchunks+1):
		bsubcom1 = 'bsub -o '+outfolder+'/OUT/cov_'+str(i+1)+'_chunk'+str(j)+'.out -e '+outfolder+'/ERR/cov_'+str(i+1)+'_chunk'+str(j)+'.err -J cov_'+str(i+1)+'_chunk'+str(j)+' -q normal -M 16000000 '
		com3 = '\'/home/grey2/bin/QTLtools1.1/bin/QTLtools cis --vcf '+vcf_file+' --bed '+phenotype_matrix+' --cov '+outfolder+'/covariates_'+str(i+1)+'.txt --permute '+str(nperm)+' --chunk '+str(j)+' '+str(nchunks)+' --out '+outfolder+'/permutations_cov'+str(i+1)+'_'+str(j)+'_'+str(nchunks)+'.txt\''
		print(bsubcom1+com3)
...
for k in covariates:
	...
	bsubcom2 = 'bsub -o '+outfolder+'/OUT/cov_'+str(k+1)+'_assemble.out -e '+outfolder+'/ERR/cov_'+str(k+1)+'_assemble.err -J cov_'+str(k+1)+'_assemble -q priority -w \'ended(\'cov_*_chunk*\')\' '
